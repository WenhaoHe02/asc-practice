...
int main(int argc, char *argv[]) {
  ...
  int SIZE[30] = {100,  200,  300,  400,  500,  600,  700,  800,
                  900,  1000, 1100, 1200, 1300, 1400, 1500, 1600,
                  1700, 1800, 1900, 2000, 2100, 2200, 2300, 2400,
                  2500, 2600, 2700, 2800, 2900, 3000}; // testing 100-3000
                                                       // square matrices
  ...
  int n_count, N = 3, upper_limit = 30;
  long double sum_gflops;
  ...
  for (int i_count = 0; i_count < upper_limit; i_count++) {
    ...
    if (kernel_num != 0) { // not an MKL implementation
      test_mydgemm(m, n, k, alpha, A, B, beta, C);
      cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, m, n, k, alpha, A,
                  m, B, k, beta, C_ref, m);
      if (!verify_matrix(C_ref, C, m * n)) {
        printf("Failed to pass the correctness verification against Intel MKL. "
               "Exited.\n");
        exit(-3);
      }
    }
    ...
    for (n_count = 0; n_count < N; n_count++) {
      if (kernel_num)
        ...
        test_mkl(m, n, k, alpha, A, B, beta, C);
    }
    t1 = get_sec();
    long double gflops = 2. * 1e-9 * N * m * n * k / (t1 - t0);
    printf("Average elasped time: %f second, performance: %.3Lf GFLOPS.\n",
           (t1 - t0) / N, gflops);
    sum_gflops += gflops;
    copy_matrix(C_ref, C,
                m * n); // sync C with Intel MKl to prepare for the next run
  }
  free(A);
  free(B);
  free(C);
  free(C_ref);
  printf("\nAvg performance: %.3Lf GFLOPS.\n", sum_gflops / 30);
  return 0;
}
